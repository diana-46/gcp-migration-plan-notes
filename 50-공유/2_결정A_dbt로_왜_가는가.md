...
> 관련: [[20-애슬론/5_Neptune SQL 변환의 dbt-BigQuery 대체 검토]], [[21-dbt/0_dbt 기본 개념]]
...
|---|---|---|
| PLAIN, no partition, 매번 재생성 | `materialized='table'` | [[21-dbt/1_materialization]] |
| PLAIN, daily partition, 재빌드 | `incremental` + `insert_overwrite` | 같은 노트 |
| YAML multi-temp + perm | `temp/` 계층 + mart | [[21-dbt/7_테이블 아웃풋]] |
| hourly-in-daily block replace | custom `insert_only` + pre_hook DELETE | [[21-dbt/8_insert_overwrite_매커니즘]] |
...
| `${var}` 치환 | `{{ var('...') }}` | [[21-dbt/4_parameter 치환]] |
| 명시적 upstream 선언 | `ref()` 자동 추론 | [[21-dbt/5_의존성 관리]] |
| 스키마 자동 sync | `on_schema_change` + `contract` | [[21-dbt/2_schema 관리]] |
...
**관련 노트**: [[20-애슬론/5_Neptune SQL 변환의 dbt-BigQuery 대체 검토]], [[20-애슬론/8_배포 시 유의할 점]], [[21-dbt/8_insert_overwrite_매커니즘]]
...
- 5-page 챗봇용 gotcha 노트 [[21-dbt/0_dbt 기본 개념]] ~ [[21-dbt/8_insert_overwrite_매커니즘]]

## 관련 문서

- [[20-애슬론/5_Neptune SQL 변환의 dbt-BigQuery 대체 검토]] — 기술 검증 결과
- [[20-애슬론/8_배포 시 유의할 점]] — 실전 배포 함정
- [[21-dbt/0_dbt 기본 개념]] — dbt 기본기
- [[21-dbt/1_materialization]] — materialization 별 trade-off
- [[21-dbt/7_테이블 아웃풋]] — 저장 전략
- [[21-dbt/8_insert_overwrite_매커니즘]] — insert_overwrite 내부 동작
