...
팀 위키가 이 레포를 읽어갈 때의 규칙(범위·신뢰도·함정)은 **[[wiki-manifest]]** 가 정본이다.
...
title: "노트 제목"
tags: [주제태그, 추가태그]
status: draft     # wip(작업중, 스크랩 제외) | draft(증언) | final(확정 — 팀 위키가 결론으로 인용 가능) | archived(폐기, 스크랩 제외)
created: 2026-05-12
updated: 2026-05-12
...
- **이미지 금지 (2026-09-08)** — 스크린샷 붙여넣기 대신 내용을 **글로 옮겨 적는다** (표·로그 발췌·수치). 이미지는 팀 위키가 스크랩하지 못해 정보가 유실된다. 구조·아키텍처 다이어그램은 **mermaid 코드블록**(텍스트라 스크랩 가능) 우선, 발표용·인터랙티브 대형 다이어그램만 HTML로 만들어 `docs/` 하위에 둔다.
