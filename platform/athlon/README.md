...
>
> 결론 노트는 [[1_개요]].
> 편입 노트 (2026-08-20, 구 `neptune/`·`memo/` 폴더에서 병합): [[9_이관대상 Operator 인벤토리]] · 10_athlonQuerySensor 관련(노트 미생성 — AthlonQuerySensor 상세는 [[7_3_공통 Custom Operator 제공 방안]] 참조)
...
- **단순 "dbt가 좋은가" 같은 일반론**은 여기 안 적음. [[../airflow/README]] 또는 별도 dbt 폴더 (필요 시) 영역.
...
- **Airflow 외 스케줄러** → [[../airflow/README]] 영역 (Airflow 유지 확정)
...
| 어디서? | 다루는 내용 |
|---|---|
| [[../airflow/README\|스케줄러/]] | Airflow 인프라(Composer vs Self-managed), **Airflow에서 dbt를 어떻게 실행** (Operator 선택 / Pod 스펙 / manifest 전달) |
