...
>
> 각 자산의 detail 은 별개 노트에 정리되어 있고 ([[7_3_공통 Custom Operator 제공 방안]] / [[athlon/PoC/02_dbt_render_in_composer]] · [[athlon/PoC/03_bq_dbt_run_in_composer]] / [[11_DAG Bundles와 배포 전략]]), 본 문서는 **세 가지를 한 장에 묶어** 통합 그림 + 의사결정 인덱스 역할.
...
> | 2 | dbt Project | DAG repo 안 `dbt_projects/` 동봉 + CI 가 `manifest.json` 생성 → GCS | [[athlon/PoC/02_dbt_render_in_composer]] · [[athlon/PoC/03_bq_dbt_run_in_composer]] |
...
[[athlon/PoC/03_bq_dbt_run_in_composer]] 의 분석에서 확인된 사내 검증 패턴:
...
### Layer 2 — dbt Project ([[athlon/PoC/02_dbt_render_in_composer]], [[athlon/PoC/03_bq_dbt_run_in_composer]])
...
- [[athlon/PoC/02_dbt_render_in_composer]] — Layer 2 렌더링 검증 (Cosmos 패턴)
- [[athlon/PoC/03_bq_dbt_run_in_composer]] — Layer 2 BQ 실제 실행 검증
